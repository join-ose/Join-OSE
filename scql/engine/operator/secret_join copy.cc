...
size_t r0 = sctx->lctx()->GetStats()->sent_actions;
...
SPDLOG_INFO("SecretJoin local time2(ms): {}", local_ms);
size_t b1 = sctx->lctx()->GetStats()->sent_bytes;
size_t r1 = sctx->lctx()->GetStats()->sent_actions; 
// if (sctx->lctx()->Rank() == 0) {
  // std::cout << "Join communication sent bytes: " << (b1 - b0)
  //           << ", sent actions: " << (r1 - r0)
  //           << std::endl;
  SPDLOG_INFO("Join communication sent bytes:{}", (b1 - b0));
  SPDLOG_INFO("sent actions:{}", (r1 - r0));
// }
