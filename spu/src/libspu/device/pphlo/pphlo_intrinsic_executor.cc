...
#include "libspu/kernel/hlo/prefix_sum.h"
...
std::vector<Value> intrinsic_dispatcher(SPUContext* ctx,
  ...
  if (name == "invperm") {
      ...
      absl::Span<const spu::Value> input_span(inputs.data(), 1);
      return spu::kernel::hal::apply_inv_permute_1d(ctx, input_span, inputs[1]);
      // auto row0 = inputs[0].row(0);  // 第一行
      // auto row1 = inputs[0].row(1);  // 第二行
      // std::vector<spu::Value> new_inputs = {row0, row1};
      // absl::Span<const spu::Value> input_span(new_inputs.data(), 2);
      // return spu::kernel::hal::apply_inv_permute_1d(ctx, input_span, inputs[1]);
  }
  ...
  if (name == "buildPerm") {
      ...
      return spu::kernel::hlo::buildPerm(ctx, inputs[0], inputs[1]);

  }
  
  if (name == "prefixTree") {
      SPU_ENFORCE(inputs.size() == 2);
      auto ret =  spu::kernel::hlo::prefix_sum(
      ctx, inputs[0], inputs[1],
      [&](const spu::Value& lhs_v, const spu::Value& lhs_gm,
          const spu::Value& rhs_v, const spu::Value& rhs_gm) {
        spu::Value new_v = spu::kernel::hlo::Add(ctx, lhs_v, spu::kernel::hlo::Mul(ctx, lhs_gm, rhs_v));
        spu::Value new_gm = spu::kernel::hlo::Mul(ctx, lhs_gm, rhs_gm);
  ...
  if (name == "soPRF") {
      SPU_ENFORCE(inputs.size() == 2);
      auto soprf = spu::kernel::hlo::SoPrf(ctx, inputs[0]);
      std::vector<spu::Value> ret;
      ret.emplace_back(spu::kernel::hlo::Concatenate(ctx, {soprf}, 0).setDtype(inputs[0].dtype()));
      return ret;
  }
