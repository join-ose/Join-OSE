...
spu::Value _gen_inv_perm_s(SPUContext *ctx, absl::Span<spu::Value const> keys,
  ...
  bv_size=32;
  // std::cout<< "bv_size: " << bv_size << std::endl;
  size_t bv_idx = 0;
...
std::vector<spu::Value> apply_inv_permute_1d(
  ...
  int k = m/n;
  std::vector<spu::Value> tmp1;
  for (int i = 0; i < k; i++ ){
    tmp1.push_back(hal::slice(ctx, inputs[0], {i*n}, {i*n + n}));
  }
  absl::Span<const spu::Value> tmp2(tmp1.data(), k);
  auto result = internal::_apply_inv_perm(ctx, tmp2, perm);
  return {hal::concatenate(ctx, result, 0)};
}
...
std::vector<spu::Value> apply_permute_1d(SPUContext *ctx,
  ...
  absl::Span<const spu::Value> tmp2(tmp1.data(), k);
  auto result = internal::_apply_perm(ctx, tmp2, perm);
  return {hal::concatenate(ctx, result, 0)};
}
