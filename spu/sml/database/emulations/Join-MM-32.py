...
def emul_Join_NN(mode: emulation.Mode.MULTIPROCESS):
    ...
    def InnerJoin(skx,sky,spx,spy,x_zeros_0,y_zeros_0,nx,ny,m):
        N = (nx+ny)*2
    ...
    try:
        ...
        kx=np.random.randint(0, 2**30,size=nx-2*n, dtype=np.uint32)
        ky=np.random.randint(0, 2**30,size=ny-2*n, dtype=np.uint32)
        ...
        kx,ky,py=emulator.run(InnerJoin,static_argnums=(6,7,8,))(skx,sky,spx,spy,x_zeros_0,y_zeros_0,nx,ny,m)
