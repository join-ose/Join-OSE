...
def emul_Join_NN(mode: emulation.Mode.MULTIPROCESS):
    ...
    def  GetPostion(s_replicate,n,m):
        ...
        e = jnp.concatenate([ones,zeros], axis=0)
        e_1 = jnp.concatenate([ones+1,zeros], axis=0)
        e = jnp.array(si.invperm(e,rho))
        e_1 = jnp.array(si.invperm(e_1,rho))
        e_2 = jnp.concatenate([jnp.ones(1, dtype=jnp.int32), e_1[1:]])
        e_2 = jax.lax.associative_scan(jnp.add, e_2)
        inv_perm = GenBitPerm(e,n+m)
    ...
    def InnerJoin1(skx,sky,spx,spy,x_zeros_0,y_zeros_0,nx,ny,m):
        ...
        # sky = jnp.concatenate([sky,y_zeros,-sky], axis=0)
        ...
        # sky = jax.lax.associative_scan(jnp.add, sky)
    ...
    try:
        ...
        # )
        # emulator.up()  
        conf_path = "examples/python/conf/2pc_semi2k.json"
        emulator = emulation.Emulator(
            conf_path, mode
        )
        emulator.up()   
        ...
        ny=2**20
        n=nx//4
        k=np.random.randint(0, 2**30,size=n, dtype=np.uint64)
        kx=np.random.randint(0, 2**30,size=nx-2*n, dtype=np.uint64)
        ky=np.random.randint(0, 2**30,size=ny-2*n, dtype=np.uint64)
        kx=jnp.concatenate([k,k,kx], axis=0)
        ky=jnp.concatenate([k,k,ky], axis=0)
        keyx = jax.random.PRNGKey(0)
        keyy = jax.random.PRNGKey(1)
        kx=jax.random.permutation(keyx,kx)
        ky=jax.random.permutation(keyy,ky)
        px=np.random.randint(0, 2**30,size=nx, dtype=np.uint64)
        py=np.random.randint(0, 2**30,size=ny, dtype=np.uint64)
        ...
        x_zeros_0 = np.zeros(m, dtype=np.uint64)
        y_zeros_0 = np.zeros(m, dtype=np.uint64)
